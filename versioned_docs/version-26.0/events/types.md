...
If the job should reschedule itself on a Finished OK or Failed status, Continuous recommends using the [Job Frequency](../job-components/frequency.md) and [Other Job Details](../job-components/frequency.md#other-job-details) features in the Concepts online help.
...
- **\[exclude notification id from header: Y/N\]**: Specifies whether to include the Notification ID in the header of the notification. This parameter is optional
...
- **Task Counts for <Customer Name from License File\>**: This section contains a comma-separated list of:
  ...
  - agent Type: The type of agent that processed the tasks
  ...
  - Following the first list, there is a comma-separated list of:
    ...
    - agent Type: The agent type that processed too many tasks
    - Total Tasks: The total number of tasks that were processed by the agent Type on the date the tasks were exceeded
- **Tasks Counted for SMA Utility Jobs**: In a comma-separated list, this section contains the list of jobs that were not counted against the task based license:
  - Period: The date or month for which the report processed
  - agent Type: The type of agent that processed the tasks
  - Number of Days with tasks: Within the agent Type, the number of days on which tasks processed in the period
  - Total Tasks in Period: Within the agent Type, the total number of tasks that processed in the selected period
  - Avg Tasks per Day: Within the agent Type, the average number tasks that processed each day in the selected period
...
- **<To\>**: One or more email address (including distribution groups), separated by semicolons (;), to receive the message
  - To generate specific task based license reporting information to send to Continuous, enter the license email address for your Continuous sales and support office:
  ...
  - If the Send Email to SMA Office value in the Global Options is set to Disabled, SAM will write the license reporting information to the SAM.log file only, and no email is sent because of the event
  - If the Send Email to SMA Office value in the Global Options has and Continuous             office selected, the license reporting information will be emailed to Continuous
- **<Cc\>**: One or more email address (including distribution groups), separated by semicolons (;), to be copied on the message
- **<Bcc\>**: One or more email address (including distribution groups), separated by semicolons (;), to be blind copied on the message
- **<Subject\>**: The subject of the email
...
OpCon events are organized by target: Job events control individual job behavior (add, hold, restart, cancel), Schedule events control schedule state (build, hold, release, cancel), Threshold/Resource events update numeric values, Property events manage user-defined properties, and Machine events control agent communication.

**Q: Can event parameters use tokens to pass dynamic values?**

Yes. Event parameters support OpCon tokens — property values enclosed in double brackets (e.g., `[[$SCHEDULE DATE]]`) — which the SAM resolves before processing the event.

**Q: How are events passed to the SAM from external sources?**

External events are passed by creating an ASCII file with the event string, User Login ID, and event password, then placing it in the SAM's MSGIN directory. Some agents can also send events directly to the SAM using a platform-specific command.
