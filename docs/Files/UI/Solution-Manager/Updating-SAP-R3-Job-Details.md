...
8. Select the **Lock** button at the top-right corner to enter **Admin** mode. The button displays as unlocked when enabled.
...
10. From the **Machines or Machine Group** list, select the **machine** where the Agent is installed. To specify a machine group, toggle the **Machines** switch to **Machine Group** and select a group from the list.
...
Select the add variant button to open the dialog. Wait for the SAP system to return **Variant Parameters**.
