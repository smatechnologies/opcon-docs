...
description: "Frequencies specify the recurring days on which a schedule or a job should run."
...
In this example, set the End of Month frequency as the highest priority. On the days the End Of Month frequency overlaps with Monday or Friday, the End Of Month frequency will be used for scheduling.
:::
...
The frequency name identifies the frequency definition. All frequency names are unique user-defined names with up to twenty characters. The frequency can be re-used on as many schedules and jobs as necessary. Continuous recommends defining a frequency naming convention for the environment so it is easy to identify frequencies for reuse. The examples below provide suggested names for some common frequencies:
...
:::note
There are some scenarios that can cause duplicate frequency names to be created. Continuous recommends resolving duplicate names with the SMA Frequency Utility. Refer to [Using Frequency Manager](../Files/UI/Enterprise-Manager/Using-Frequency-Manager.md) in the **Enterprise Manager** online help.
:::
...
- **On Occurrence**: The frequency defines an occurrence of a day in a month, quarter, or year. For example, a frequency can be defined for the third Tuesday and third Thursday of each month
  ...
  - Frequency Calculations:
    ...
    3. If the Day Type is set to Working, and a date in the list of qualifying dates is on a non-working day, OpCon applies the [A/O/B/N setting](#aobn). For more on non-working days, refer to [Holiday Calendars](../objects/calendars.md#holiday-calendars)
...
- **On Request**: The frequency defines a specific date. This group may occur only one time or annually
  ...
  - Frequency Calculations:
    ...
    2. If the qualifying date is on a non-working day, OpCon applies the [A/O/B/N setting](#aobn). For more on non-working days, refer to [Holiday Calendars](../objects/calendars.md#holiday-calendars)
...
- **On Day**: The frequency defines the Nth day of a month. Valid values for the day range from 1 to 31
  ...
  - Frequency Calculations:
    ...
    2. If a Calendar is associated, OpCon matches the dates in the Calendar to the initial list of qualifying dates and removes any matching dates from the list. For more information, refer to [Negative Annual Plan Calendars](../objects/calendars.md#negative-annual-plan-calendars)
    3. If the Day Type is set to Any, and a date in the list of qualifying dates is on a non-working day, OpCon applies the [A/O/B/N setting](#aobn). For more on non-working days, refer to [Holiday Calendars](../objects/calendars.md#holiday-calendars)
    4. If there are any Advanced Frequency options defined for a job, those options override all other calculations. For more information, refer to [Advanced Frequency Settings](#advanced-frequency-settings)
...
- **Beg of Period**: The frequency defines a day on or around the beginning of a period. The first day of the period depends on the Day Type:
  ...
  - Frequency Calculations:
    ...
    2. If the Day Type is set to Working, and a date in the list of qualifying dates is on a non-working day, OpCon applies the [A/O/B/N setting](#aobn). For more on non-working days, refer to [Holiday Calendars](../objects/calendars.md#holiday-calendars)
...
The Request Date determines the date on which the frequency will qualify. Enable the every year option to indicate the request is made annually on a day of a month.
...
- Valid options for the A/O/B/N setting include:
  ...
  - **Before Date (B)**: Moves the job to the first working day before the scheduled day
...
**Q: What is the difference between a schedule frequency and a job frequency?**

A schedule frequency applies to all jobs on a schedule and affects every existing and future job in that schedule. A job frequency applies only to the individual job it is defined for.

**Q: What happens when multiple frequencies qualify for the same day?**

OpCon checks frequencies in priority order, highest first. The first frequency that qualifies for the build date is used, and all lower-priority frequencies are ignored for that date.

**Q: Can a frequency be shared across jobs?**

Yes. A named frequency can be reused on multiple jobs. Changes to a shared frequency affect all jobs using it.

**Q: Does Include in Schedule override Exclude from Schedule?**

Yes. When the same date appears in both Include and Exclude parameters, the Include setting takes precedence and the job is scheduled on that date.
