...
- Use menu path: **Start \> All Programs \> OpConxps \> Utilities \> Schedule Import Export**. This utility requires the same User Login ID and password as Enterprise Manager. For information on logging in to Enterprise Manager, refer to [Logging In/Out](../../Files/UI/Enterprise-Manager/Logging-In.md) in the **Enterprise Manager** online help
...
2. Do the following in the **Password** text box:
    ...
    :::
To log in to Schedule Import/Export, complete the following steps:

3. Select the **OpCon database** in the **Data Source** field
4. Select the **Save Settings** option to save the connection information for the next login and select **OK**
...
:::note
If the contents of the Access database need to be preserved, the .mdb file should be copied to another file/directory. Do not rename the .mdb file in the Utilities directory because the program does not can create a new Access database template. It is a good practice to maintain a backup copy of the original IMPEX.MDB file.
:::
...
- When a new machine is created in the destination database, the Schedule Import/Export utility will mark the machine status "Down" to allow the management of the agents within the license. The machine will need to be marked "Up" for jobs to process on it
...
:::note
If two schedules have cross-schedule job dependencies and both are migrated, it is better to migrate them at the same time to maintain the cross-schedule dependencies. If the schedules are migrated separately, the job dependency references are lost because they cannot be resolved.
:::
...
2. Unselect the **Use Master Holiday** option
3. Select **Save** ![Save icon](../../Resources/Images/Utilities/EMsave.png "Save icon") on the **Schedule Master** toolbar
...
8. In the ODBC Database section on the lower part of the window:
    - Select one schedule at a time to select one or more individual schedules or export. Alternatively, you can select one schedule then press **Ctrl+A** on the keyboard to select all
9. Below the toolbar:
10. If desired, select the **Force Machine Mapping** option
11. On the toolbar:
12. Select the **Export to Transport Database** button
    :::note
...
14. Select the **Yes** or **No** button when asked if the target database should be cleared
    - If **Yes**, the target database is cleared and the new schedule(s) transported. Select **OK** when the completion message is displayed
    - If **No**, the process is canceled
15. Select the **Yes** or **No** button when asked if all tokens should be transferred
...
To export Schedules, complete the following steps:

16. Select the **Yes** or **No** button when asked if all thresholds/resources should be transferred
...
4. In the Transport Database section on the upper part of the window:
    - Select one schedule to automatically select all schedules to import
5. On the toolbar:
    - Select the **Import from Transport Database** button
6. For each message box:
    - Select the **Machine** or **User** tab
    :::note
    ...
    :::
7. Go to the left-hand frame and select a **machine or User Account**
8. Go to the right-hand frame and select a **machine or user name**
:::note
...
11. Select the appropriate answer if prompted to overwrite an existing schedule or calendar
...
- **-e**: (Optional) The full file path (including drive letter) and filename where the Exit Code Override File (E.C.O.F.) is written. If not specified, a file called ImpEx.dat will be placed in the same directory as the ImpEx.exe file. The Exit Code Override File is used by the Microsoft agent to determine the success or failure of the job. The Microsoft agent will compare the failure criteria to the E.C.O.F. file instead of the standard exit condition of the job. For Schedule Import Export exit codes, refer to the [Exit Codes](#Exit) table. When the job is defined in Enterprise Manager, make sure to specify the E.C.O.F. path and file name as specified by -e parameter, or the job will always be considered a success
...
| Select Access DSN | Allows users to change the transport database connection | — | — |
| Import from Transport Database | Import the selected schedule(s) from transport database to the SQL database | — | — |
| Export to Transport Database | Export the selected schedule(s) from the SQL database to the transport database. | — | — |
| Force Machine Mapping | Determines if machine mapping should be required during the import process. | — | — |
| ImpEx.exe | The name of the schedule import export program | — | — |
| AccessDSN | The ODBC Data Source Name corresponding to the Access Database used as the transport database | — | — |
| SQLDSN | The ODBC Data Source Name corresponding to the OpCon database. | — | — |
| UID | A valid, case-sensitive OpCon User Login ID | — | — |
| PWD | The case-sensitive password corresponding to the User Login ID above | — | — |
| GlobalProperties? | Used to indicate which Global Properties to export from the SQL database. | — | — |
| Thresholds? | Used to indicate whether to transfer all of the thresholds and resources from the SQL database or not. | — | — |
| ScheduleName(s) | The name(s) of the schedule(s) to be exported | — | — |
| -e | The full file path (including drive letter) and filename where the Exit Code Override File (E.C.O.F.) is written. | — | — |
| -f | (Optional) The full file path (including drive letter) and filename containing a list of schedules for the program to extract | — | — |
| PurgeJobs? | Used to indicate how the import should continue if any one of the schedules in the transport database already exists in the SQL database | — | — |
| OverwriteCal? | Used to indicate how the import should continue if any one of the calendars in the transport database already exists in the SQL database | — | — |
| -m | The full file path (including drive letter) and file name for the mapping file. | — | — |
| Mapping Type | Involves one of two options: | — | — |
| Source Item Name | The name of the Machine or User as it exists in the transport database | — | must be a matching OS type with a matching data type (XML or Non-XML) for the import to |
| Destination Item Name | The name of the Machine or User to map the source item to in the destination database | — | must be a matching OS type with a matching data type (XML or Non-XML) for the import to |
## Operations

### Common Tasks
- Export schedules using the **Export to Transport Database** toolbar button or `ImpEx.exe ... EXPORT ...` from the command line; the transport database is cleared with each export.
- Import schedules using the **Import from Transport Database** toolbar button or `ImpEx.exe ... IMPORT ...`; all schedules in the transport database are imported together and the entire transaction rolls back on any failure.
- When importing to a new environment, use the machine and user mapping dialog (or the `-m` mapping file on the command line) to map source machines and users to their destination equivalents.
- If the transport database needs to be preserved between export runs, copy the `.mdb` file to another location before exporting; the utility does not support merging new schedules into an existing transport database.
...
**Disabled user account prevents login and shuts down the utility** — If the user login ID defined as a Network Account name is disabled, the Import Export utility informs the user they are disabled and shuts down the program — Re-enable the account in OpCon user management or use a different account that has "All Administrative Functions" or "Import and Export Schedules" privileges.

**Duplicate job names during merge cause the entire transfer to roll back** — If a conflict arises where duplicate job information exists during a merge of source and target schedules, an error message is displayed and the entire transfer operation is terminated with an error; all changes are rolled back — Ensure there are no duplicate jobs across the schedules before attempting a merge, or choose to clear (not delete) the existing schedule object to receive new information rather than merging.

**Missing cross-schedule job dependencies generate a warning** — If imported schedules have job dependencies on schedules that do not exist in the target database, a warning is displayed — Select OK to ignore the missing dependency and continue the transfer, or select Cancel to terminate the transfer and resolve the dependency before retrying.

**Batch User IDs missing in destination database cause jobs to fail** — Batch User IDs submitted with jobs are only imported if those IDs exist in the destination database; jobs fail if no valid Batch User ID is listed — Verify and set up required Batch User IDs in the destination database before importing, then review all imported job details after the import completes.

**Schema version mismatch prevents schedule import** — The schema versions of the source and target databases must be the same for the import to succeed — Ensure both the source and destination databases are running the same OpCon schema version before attempting the transfer.

**Force Machine Mapping required but mapping file not provided causes import failure** — When the transport database has Force Machine Mapping enabled and no mapping file is provided via `-m`, the import fails and the log file indicates that the required mapping was not defined — Provide a valid mapping file using the `-m` parameter that maps all machines; if any machine is not mapped, the import will also fail.
...
The utility requires appropriate ODBC drivers for the SQL database (not provided by OpCon installation) and Microsoft Access ODBC drivers for the transport database (included in the OpCon installation).
