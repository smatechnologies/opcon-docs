...
SMA Evaluate Expression (SMAEvalExp.exe) accesses the OpCon Property Expressions API — the same API used by Job Dependencies and Events Based on Complex Expressions. See [Property Expressions API Syntax](../../reference/property-expressions-syntax) for details. The utility can be scheduled from an OpCon Windows job or run from a Windows command prompt (cmd.exe).

:::note Legacy utility
The standalone SMAEvalExp.exe ships only with legacy LSAM-style installations and is not included in the current core installer. It is retained for backward compatibility. The supported in-product equivalent is the Property Expressions feature, used in job dependencies and events based on complex expressions.
:::

SMAEvalExp.exe is installed in the `<Target Directory>\OpConxps\MSLSAM\` directory and connects to the OpCon database using the .dat file generated by the SMA Connection Configuration utility. See [SMA Connection Configuration Tool](../../server-programs/schedule-activity-monitor.md#sma-connection-configuration-tool) for details.
...
- **Expression**: A logic, numeric, or property-assignment expression
  ...
  - If the expression contains embedded tokens, wrap it in the `[[=]]` syntax. See [Property Expressions API Syntax](../../reference/property-expressions-syntax) for details

### Examples

The following examples use [Operators](../../reference/property-expressions-syntax#operators), [Functions](../../reference/property-expressions-syntax#functions), and [System Properties](../../objects/properties#system-properties) to demonstrate common automation uses:
