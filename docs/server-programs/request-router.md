...
The SMA Request Router reads the OpCon database to process requests from the SAM and Solution Manager. It retrieves records, sends them to the designated handler, and writes completion information back to the OpCon database.
...
SMA Request Router reads the OpCon database to process requests from the SAM and Solution Manager. It retrieves records, sends them to the designated handler, and writes completion information back to the OpCon database.
