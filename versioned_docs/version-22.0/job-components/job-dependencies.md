...
|Finished OK|If selected, this indicates that the dependency can be resolved when the dependent job finishes OK. A skipped job satisfies this dependency as if it finished OK. This option is only available when the After or Requires dependency type are defined.|
