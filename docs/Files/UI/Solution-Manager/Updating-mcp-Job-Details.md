...
:::note
Only users with the appropriate permissions have access to the **Lock** button and can update job properties. For details about privileges, refer to [Required Privileges](Accessing-Daily-Job-Definition.md#Required) in the **Accessing Daily Job Definition** topic.
:::
...
:::note
Changes to job properties in the **Daily Job Definition** take effect immediately. If the job has already run, changes take effect the next time the job runs.
:::

To access the MCP job daily job definition, complete the following steps:
...
This page covers updating MCP Job Task Details, Pre-Run Information, and Failure Criteria for MCP jobs in the daily job definition.
