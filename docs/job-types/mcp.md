...
MCP job details define the parameters for running a job on a Burroughs MCP (Master Control Program) agent. Text fields in the graphical interfaces support OpCon token replacement. [Tokens](../objects/using-properties.md#tokens) are resolved as the SAM prepares to send the job to the agent. For more information about this platform, refer to [MCP Agent Configuration](https://help.smatechnologies.com/opcon/agents/mcp/latest/Files/Agents/MCP/Configuration.md) in the **MCP Agent** online help.
...
**Job Summary Information**: For START type jobs, specifies a custom location for job summary information — useful for sites that store job summaries outside the MCP agent default location. The JORS process uses this information when a JORS request is sent.
...
:::note
For information on cleaning up files generated by the MCP Agent when running EAE/AB Suite jobs, refer to [Using \*SMA/WFL/CLEANUP/LINC17/FILES](https://help.smatechnologies.com/opcon/agents/mcp/latest/Files/Agents/MCP/SMA_WFL_CLEANUP_LINC17_FILES.md) in the **MCP Agent** online help.
:::
...
:::note
Effective with MCP Agent 16.02, you can modify the \*SMA/WFL/REMOVEJOB WFL to complete OK even when no files are deleted. Security errors and locked files still cause REMOVEJOB to fail.

To implement this behavior, modify a working copy of \*SMA/WFL/REMOVEJOB: comment out sequence #26600 and un-comment sequence 26650. Re-apply this modification after each MCP Agent upgrade if the alternate behavior is required.
:::
...
- **Fail Codes**: Words compared to the MCP console display. If matched, the agent follows the failure logic in the configuration. Refer to [Set up Failure and Status Message Logic](https://help.smatechnologies.com/opcon/agents/mcp/latest/Files/Agents/MCP/Set-up-Failure-and-Status-Message-Logic.md) in the **MCP Agent** online help
...
- **Fail Reset**: Words compared to the MCP console display. If matched, the agent follows the failure logic in the configuration. Refer to [Set up Failure and Status Message Logic](https://help.smatechnologies.com/opcon/agents/mcp/latest/Files/Agents/MCP/Set-up-Failure-and-Status-Message-Logic.md) in the **MCP Agent** online help
...
| Job Summary Information | For START type jobs, specifies a custom location for job summary information — useful for sites that store job summaries outside the MCP agent default location. | location | Maximum 20 characters.  **Directory or File Name**: The dire |
...
- For START-type jobs, use the **Job Summary Information** section to specify a custom location (Family and Directory/File Name) for job summary data if stored outside the MCP agent default location. The maximum length for Directory or File Name is 200 characters.
