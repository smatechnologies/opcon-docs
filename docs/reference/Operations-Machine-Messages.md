...
| MISSING OBJECT | The Details screen in Enterprise Manager contains the RUN command, but the file named in the File Title is not present. Verify the USERCODE, FAMILY, and spelling. The usercode does not have a family assignment and the Job Details File Title does not include the family name of the program. |
| NO JOB TITLE | No file title was defined on the Details screen in Enterprise Manager. Note: The PARAM/ATTR ERR message replaces NO JOB TITLE when the "Contemporary, XML" protocol is in use. |
| NO WFL FILE | The Details screen in Enterprise Manager contains the START command, but the file named in the File Title is not present. Verify the user code, family, and spelling. The mix number of this status description belongs to the agent task that initiates the WFL compile. It is not the mix number for the WFL compile itself. The usercode does not have a family assignment and the Job Details File Title does not include the family name of the WFL. Note: The PARAM/ATTR ERR message replaces NO WFL FILE when the "Contemporary, XML" protocol is in use. |
| NOT CODE FILE | The Details screen in Enterprise Manager contains the RUN command, but the File Title is not the name of an existing executable file. |
...
| NOT JOB FILE | The Details screen in Enterprise Manager contains the START command, but the File Title is not the name of an existing WFL file. |
...
| PARAM/ATTR ERR | An OpCon job initiation request (TX1) was received by the agent, but the initiation attempt failed because of a parameter mismatch or invalid attribute (job name does not conform to the rules for entity names). If the START command was used, the mix number of this status description belongs to the agent task initiating the WFL compile, and not to the mix number for the WFL compile itself. A possible cause for this error is that the Details screen in Enterprise Manager contains the START or RUN command, but the file named in the File Title is not present. Verify the USERCODE, FAMILY, and spelling. |
| PARAMETER ERR | The Details screen in Enterprise Manager contains the RUN command, but a parameter is erroneous. Verify the parameter length and type. |
...
| USERCODE ERROR | The agent was unable to completely assume the identity (e.g., batch user privileges, family assignments, and so forth) of the usercode specified on the Details screen in Enterprise Manager. Verify the spelling. |
...
| WFL SYNTAX ERR | The WFL source file contains syntax errors. Correct the WFL source. The File Title field on the Details screen in Enterprise Manager may contain extraneous characters, such as CR/LF, START, or RUN. The File Title field should contain only the file title of the process to be initiated; furthermore, if the job is being initiated using a RUN command, the appropriate task attributes should also be included. The WFL source file contains an "INCLUDE" statement that references a file that cannot be found. The WFL compile was DS'ed. |

## OS 2200 Job Messages

The table below presents OS 2200 Agent status messages forwarded to the SAM. The messages are displayed in Schedule Operations of the graphical interfaces following the job status.
...
### OS 2200 Agent Messages Displayed at System Console
...
| agent-LOCK FILE NOT FOUND ** | The XFRTCP has detected the agent-LOCK file is not catalogued. Catalogue the <Agent-qualifier\>*agent-LOCK file to resolve. |
...
| 70007 | Error in Job definition get: after the job was copied for running, the agent couldn't retrieve the copied job's details; therefore, the job could not run. |
...
| XPS028I | Sysout Agent Ready on Port:*nnnn*-*JESx* | - Indicates Enterprise Manager Sysout View agent (XPSYSOUT) sucessfully opened the IP communication port (*nnnn*) for SYSOUT requests. <br />- *JES* displays the SAPI subsystem from which SYSOUT is retrieved. |
...
| XPS050E | XPSCOMM PARM or MSGIN Error | - Indicates no parm was supplied on the XPSCOMM run statement and the MSGIN DD could not be opened.<br />- Supply a PARM=, PARMDD=, or MSGIN DD for the job step. |
| XPS051E | XPSTRACK Tracking Error | - Indicates the XPSTRACK module encountered an error attempting to update the tracking record of the job in question.<br />- Contact Continuous Support if this message persists or the reason for its issuance cannot be determined. |
| XPS051W | Userid override not authorized for \[*userid*] | - Indicates an XPSCOMM caller referenced an event with a Security ID override, but the override could not be validated by the security subsystem.<br />- The override is not applied, and the event is sent with the calling user's own identity.<br />- Grant the calling user READ access to either the \[*userid*].SUBMIT or \[*userid*].OPCON profile in the SURROGAT class. |
| XPS052I | XPSCOMM Processing Successful | - Indicates XPSCOMM completed processing.<br />- This message does not confirm that OpCon received the input. XPSCOMM also issues it when the agent is not active and when an unauthorized run discards input that exceeds 117 characters. |
