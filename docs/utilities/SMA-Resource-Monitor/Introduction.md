...
The SMA Resource Monitor is a Windows utility that monitors files and Windows counters. When a monitored condition is met, it sends OpCon events for processing.
...
- **File Monitors** — trigger when a file matches defined criteria
- **Counter Monitors** — trigger based on Windows performance counter values
- **Service Monitors** — trigger based on the state of a Windows service
- **Process Monitors** — trigger based on whether a process is running
